 ...
 */
var account_type_list_table = $("#account_type_list_table");
...
function loadAccountTypeList() {
	...
	account_type_list_table.datagrid({
		title : "账户类别信息",
		method : "post", // get方式可以查看easyui封装的分页参数
		url : "../business/accountType/getAccountTypeList",
//		queryParams : serializSearchForm(), // TODO 使用jQ序列化表单数据会乱码,暂时采用这种方式
		queryParams : $("#account_type_search_form :input").serializeArray(), // TODO 使用jQ序列化表单数据会乱码,暂时采用这种方式
		fit : true,
		sortName : 'accountTypeCode',// 排序字段(传递后台)
		...
		toolbar : [ {
			...
			handler : function() {
				...
				var row = account_type_list_table.datagrid("getSelected");
				if (null == row) {
	...
	$("#account_type_popup_div").dialog({
		...
		buttons : [ {
			...
			handler : function() {
				...
				saveAccountType();
				$("#account_type_popup_div").dialog("close");
				accountTypeListFlush();
			}
		}, {
			text : "取消",
...
function resetAccountType() {
	...
	$.each($("#account_type_search_form input"), function(index, obj) {
		// alert(typeof(obj));
...
function accountTypeListFlush() {
	...
	// account_type_list_table.datagrid("load",
	// $("#account_type_search_form").serialize() );

	account_type_list_table.datagrid("load", serializSearchForm());
}
...
function serializSearchForm() {
	var jsonSearch = {
		"search_account_type_code" : $("#search_account_type_code").val(),
		"search_account_type_name" : $("#search_account_type_name").val()
	};
